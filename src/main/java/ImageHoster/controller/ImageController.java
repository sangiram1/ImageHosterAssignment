...
public class ImageController {
  ...
  //This method is called when the details of the specific image with corresponding title are
  // to be displayed
  //The logic is to get the image from the databse with corresponding title. After getting the image
  //from the database the details are shown
  ...
  //Modified by Sangeeta as part of Part A : Bug-Fix#1 : Error while viewing details of image
  //with same title.
  //@RequestMapping("/images/{title}") // Commented as part of Part A : Bug-Fix#1
  //public String showImage(@PathVariable("title") String title, Model model) {
  // Commented as part of Part A : Bug-Fix#1
  @RequestMapping("/images/{id}/{title}")
  public String showImage(@PathVariable("id") Integer imageId, @PathVariable("title") String title,
      Model model) {
  ...
  //The method receives all the details of the image to be stored in the database, and now the image
  //will be sent to the business logic to be persisted in the database
  //After you get the imageFile, set the user of the image by getting the logged in user from
  // the Http Session
  ...
  //After storing the image, this method directs to the logged in user homepage displaying
  // all the images

  //Get the 'tags' request parameter using @RequestParam annotation which is just a string of
  // all the tags
  //Store all the tags in the database and make a list of all the tags using the findOrCreateTags()
  // method
  //set the tags attribute of the image as a list of all the tags returned by the findOrCreateTags()
  // method
  ...
  //This method fetches the image with the corresponding id from the database and adds it to
  // the model with the key as 'image'
  //The method then returns 'images/edit.html' file wherein you fill all the updated details
  // of the image
  ...
  public String editImage(@RequestParam("imageId") Integer imageId, Model model,
    ...
    if (imageOwnerId.intValue() == loggedInUserId.intValue()) {
      String tags = convertTagsToString(image.getTags()); //edit.html needs tags appended by commas
      model.addAttribute("tags", tags);
      return "images/edit";
    } else {
  ...
  public String editImageSubmit(@RequestParam("file") MultipartFile file,
    ...
    // Modified by Sangeeta as part of Part A : Bug-Fix#1 : Error while viewing details of image
    // with same title
    //Since the RequestMapping has been modified for showImage method, this change is needed here
  ...
  //The method calls the deleteImage() method in the business logic passing the id of the
  // image to be deleted
  ...
  public String deleteImageSubmit(@RequestParam(name = "imageId") Integer imageId, Model model,
    ...
    Integer loggedInUserId = loggedInUser.getId();
    Integer imageOwnerId = image.getUser().getId();
    if (loggedInUserId.intValue() == imageOwnerId.intValue()) {
      imageService.deleteImage(imageId);
  ...
  // the ‘tags’ string to a list of all the tags and also stores the tags in the database if they
  // do not exist in the database. Observe the method and complete the code where required
  // for this method.
  ...
  private String convertTagsToString(List<Tag> tags) {
    ...
    /*Modified by Sangeeta as part of workaround to resolve exception caused due to no tag assigned
